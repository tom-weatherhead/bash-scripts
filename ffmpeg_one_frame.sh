...
usage()
{
	...
	echo_error_message
	echo_error_message "Usage: $PROGRAM_NAME -s StartTime [-q Quality] InputFilename"
	echo_error_message
	echo_error_message "E.g. $PROGRAM_NAME -s 13:37 foo/bar/video.mp4"
	echo_error_message "E.g. $PROGRAM_NAME -s 13:37 -q 2 foo/bar/video.mp4"
	echo_error_message
}
...
OUTPUT_FILENAME="${FILENAME}_${START_TIME_STRING}_q${QUALITY}.${OUTPUT_EXTENSION}"

# Bash's printf %q escapes the special characters in the file paths.
echo_and_eval $(printf "ffmpeg -ss $START_TIME -i %q -vframes 1 -q:v $QUALITY %q" "$1" "$OUTPUT_FILENAME")
